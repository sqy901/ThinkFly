...
while True:
    ...
    if zhenshu < 270:
    ...
    elif zhenshu < 520 or zhenshu > 705:
        alpha = 1.4  # 对比度
        beta = 10  # 亮度
        image = frame.copy()
        image = cv.convertScaleAbs(image, alpha=alpha, beta=beta)
        image_gray = cv.cvtColor(image, cv.COLOR_BGR2GRAY)
        _, mask = cv.threshold(image_gray, 140, 255, cv.THRESH_BINARY_INV)
    else:
        alpha = 0.9  # 对比度
        beta = 0  # 亮度
        image = frame.copy()
        image = cv.convertScaleAbs(image, alpha=alpha, beta=beta)
        image_gray = cv.cvtColor(image, cv.COLOR_BGR2GRAY)
        _, mask = cv.threshold(image_gray, 110, 255, cv.THRESH_BINARY_INV)
    ...
    for i in range(len(contours)):
        ...
        for j in range(len(tar_contours)):
            ...
            if not roi.any():
                continue
            _ = cv.imwrite("generate_image1/{}.png".format(cnt), roi)
            cnt += 1
            # cv.imshow("roi", roi)
            # cv.waitKey(0)
            # cv.destroyAllWindows()
...
for i in range(cnt):
    img = cv.imread("generate_image1/{}.png".format(i))
    img_gray = cv.cvtColor(img, cv.COLOR_BGR2GRAY)
    if i < 530 or i > 1826:
        _, res = cv.threshold(img_gray, 170, 255, cv.THRESH_BINARY)
    else:
        _, res = cv.threshold(img_gray, 220, 255, cv.THRESH_BINARY)
    ...
    res = img[y:y+h, x:x+w]
    _ = cv.imwrite("generate_image1/{}.png".format(i), res)
